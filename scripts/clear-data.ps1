# 清除 Input IME 用户数据（词频/学习记录），保留输入法本体与注册。
# 默认无需管理员；加 -Dict 同时删除部署词库（输入法回到透明模式，此时需提权）。
# 用法：scripts\clear-data.ps1            # 仅清用户数据（%LOCALAPPDATA%\InputIME）
...
$userDataDir = Join-Path $env:LOCALAPPDATA "InputIME"
...
if ($Dict) {
    ...
    $dictFile = Join-Path $env:ProgramData "InputIME\input.imedic"
