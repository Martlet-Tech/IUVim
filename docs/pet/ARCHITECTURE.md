...
- 全屏游戏检测：**已实现**（2026-09-01，见 `docs/status.md` 同名台账条目）——工具栏显隐第三
  维度「全屏抑制」：1 秒轮询判定前台窗口是否覆盖所在显示器整屏，命中即隐藏工具栏，桌宠同窗
  随之隐藏且 `kill_pet_timer()` 停帧（Wallpaper Engine 模式）。设置页「全屏时自动隐藏工具栏
  与桌宠」开关默认开启。
