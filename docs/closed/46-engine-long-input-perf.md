...
> 状态：**已结案**（2026-09-10）。P1（切分改词库整跨词反查，删方案枚举，`e070638`）+ P2（桶收集改
> 词典区间游标 + 非段首起点按 `max_homophones` 裁剪，`c9b88d3`）**已并入 main**；但 §5 的验收计时
> **未达目标**——残留尖峰全部落在 `onkey.buckets`，与串长无关。实施与实测实况见 §11，残项另立项。
...
## 11. 实施与验收实况（2026-09-10 结案记录）

**已实施并入 main**：P1 `e070638`（切分改整跨词反查 + preedit 职责重划）、P2 `c9b88d3`（桶收集改
词典区间游标）。基线对拍：39 号 §15A 十二条 + 长串哨兵的 `SEG/CANDS/READING` 输出与 B0 **逐字节
一致**；单键峰值 5.42s → 0.43s（P1 完成后即达成"尾型串单键 < 10ms"量级的数量级改善）。

**验收计时未达 §5 目标**（真机 notepad 复测，99 键；`%TEMP%\iuv-tsf.log` 当日）：

| 指标 | §5 目标 | 实测 |
|---|---|---|
| 尾开放键单键尖峰 | < 15ms | **未达**：p50 14.3ms / p90 71ms / max **91.9ms**；34 键 >20ms、17 键 >50ms |
| 哨兵②逐键 translate 累计 | < 1s | **未达**（分项见下） |

分项（同一次会话 97 个 `onkey.*` 采样，累计）：

| 阶段 | sum | max |
|---|---|---|
| `buckets` | **2464ms** | **90012us** |
| `assemble` | 46ms | 1128us |
| `graph` | 20ms | 421us |
| `seg` | 2.7ms | 180us |

**定性结论**：P1 把切分（含 preedit 二跑）**彻底移出瓶颈**（seg 已降到 µs 级，任务书原定目标达成）；
残留尖峰**全部在 `buckets`**，与串长无关、约四成键命中——与 §10 的「缺页/工作集颠簸」判断一致，
说明 P2 的游标化消掉了**单价**但没消掉**尖峰形态**。

**残项**：`buckets` 尖峰 → 另立项跟进。**测量注意**：复测时 `config.json` 为 `perf_probe: true`，
每键多 5 行日志写入（`log.rs` 自注"实测浏览器打字明显变卡"），上表数字含该 I/O 成本，
评估手感前应先关掉 `perf_probe`。
