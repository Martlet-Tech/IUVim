...
本目录只保留**活跃**任务书；已结案的迁移至 `docs/closed/`（10/11/12/13/14/15/17/18/19/20/21/22/24/25/26/28/31/32/40 等均已在 closed）。

| 文件 | 内容 |
|---|---|
| `01-contract.md` | **共享契约**：依赖版本、全部公共 API、行为契约、文件属主矩阵、词典二进制格式 |
| `02-conventions.md` | 全局约定：代码风格、错误处理、日志、测试纪律（原编号 30，2026-08 改号） |
| `39-rime-pipeline.md` | rime 管线改造（实施中）：统一打分/音节图/翻译器 + 收尾项 |
| `41-keymap-settings.md` | 快捷键双槽设置 + 全局热键（已并入 main，2026-08-28） |
| 其余（16/23/27/29/30/33/38） | 各活跃专题任务书，见文件头部状态行 |
