...
> 状态：**已结案**（2026-08-19 手测通过、并入 main）。前置阅读：`01-contract.md`、`02-conventions.md`、
...
- 手测（Windows notepad）：繁体模式 `nihao`→候选/上屏「你好」繁体场景（如 `yihou`→以後、
  ...
  数据文件删除后降级简体不崩。
  **已通过（2026-08-19）**：转换器装配 53250 词条（日志 `简繁转换器装配成功`）；设置页切繁体
  （配置纪元 7 热载）前后同一 notepad 会话 `wangluo` 分别 commit `网络`→`網絡`（繁体生效）；
  繁体下重打 `wangluo` 出 `網絡`（自造词录简体 + 显示转换）；验收输入 `我测试成功 读log证实`
  逐字 commit 全正常、无降级无 panic。

## 8. 待确认（实现前问用户）——已答复（2026-08-19）

1. `iuv.opencc` 是否要单独编译格式，还是允许运行时直接解析两个文本文件
   （数据量小，`<50ms` 可接受）？——用户答复：**编译二进制**（dictc 扩展，与词库管线一致）。
   已实现：IUVOCC01 格式 + `dictc opencc` 子命令。
2. daemon 设置页是否暴露简体/繁体选择（可复用现有 settings 枚举页）？
   ——用户答复：**已有**（"现在已经有了呀"，28 已落地 radio），无需新增 UI；
   仅改 settings.rs 文案「仅记录默认值」→「已生效」。
