# 11 · 任务书 B：ime-core（引擎）

> 属主文件：`crates/ime-core/src/{schema,lm,viterbi,rerank,store,engine,session}.rs`、`crates/ime-core/tests/**`
...
```
cargo test -p ime-core        # 全绿
cargo check -p ime-core       # 无 warning
```

## 6. 子智能体启动提示词

```
你负责实现 Input 输入法 MVP 的 ime-core 模块（引擎：全拼切分/候选生成/unigram Viterbi/会话状态机/排序管线）。
先读 D:\Projects\input\docs\plan\00-overview.md、01-contract.md、30-conventions.md，
再读任务书 D:\Projects\input\docs\plan\11-mod-ime-core.md 并严格执行。
接口签名与行为以 01-contract.md §4/§4.1/§4.2 为唯一权威；只能创建/修改属主矩阵中 Agent B 的文件。
candidate.rs/config.rs/key.rs/lib.rs 已冻结禁止修改；Dict 由 ime-data 提供，测试用 Dict::from_entries 构造，禁止读真实词库文件。
crate 必须保持跨平台纯 Rust（禁止 windows API）。
完成后必须满足 DoD：cargo test -p ime-core 全绿、cargo check -p ime-core 无 warning。
