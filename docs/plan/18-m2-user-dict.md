...
> 状态：**实现待测**（2026-08-14，分支 `feat/m2-user-dict`）。键位演进：初稿 Alt+←/→
...
**简单线性格式 `IUVUSR01`**（实现定稿：小文件不 mmap 零拷贝、覆盖表内存 BTreeMap——
...
> 屏蔽表（段2）/ 自造词表（段3）：预留段类型在 IMEDIC02 侧（未知段忽略）；
> 用户库侧未来升 `IUVUSR02` 追加段，读侧按 magic 分派。
...
- 钉选快捷键：被 Shift+←/→ 提至固定位 + 不再调整即为钉选效果；显式"锁死"交互待定
...
- [ ] 用户库写盘失败的重试策略（当前：失败静默，内存态生效，下次调整重试——实测后评估）
