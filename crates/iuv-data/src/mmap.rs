...
mod imp {
    ...
    impl Mapped {
        ...
        fn map(file: HANDLE, path: &Path) -> io::Result<Mapped> {
            ...
            // SAFETY: file 句柄有效。
            let mut size: i64 = 0;
            unsafe { GetFileSizeEx(file, &mut size) }.map_err(|e| {
                err(
                    io::ErrorKind::Other,
                    format!("查询词库大小失败 {}: {}", path.display(), e.code()),
                )
            })?;
            ...
            if len == 0 {
                return Err(err(
                    ...
                    format!("词库为空: {}", path.display()),
                ));
            }
            // SAFETY: 只读映射整个文件；句柄由本对象持有。
            let mapping =
                unsafe { CreateFileMappingW(file, None, PAGE_READONLY, 0, 0, PCWSTR::null()) }
                    .map_err(|e| {
                        err(
                            io::ErrorKind::Other,
                            format!("创建映射失败 {}: {}", path.display(), e.code()),
                        )
                    })?;
            let result = (|| {
...
impl MappedFile {
    pub fn open(path: &Path) -> io::Result<MappedFile> {
        Ok(MappedFile {
            repr: Repr::Mapped(imp::Mapped::open(path)?),
        })
    }
