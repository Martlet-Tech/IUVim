...
//! 配置来源：默认值 → `%LOCALAPPDATA%\InputIME\config.json`（缺省字段自动补默认）。
...
impl Config {
    /// 加载配置：`%LOCALAPPDATA%\InputIME\config.json`。
...
/// 默认配置路径：%LOCALAPPDATA%\InputIME\config.json（与词库同目录）。
...
pub fn default_config_path() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(base).join("InputIME").join("config.json"))
}

/// 配置日志：ime-core 无日志设施，仅在失败时静默（输入法场景由 TSF 层日志覆盖）。
...
mod tests {
    ...
    fn tmp_file(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("ime-config-test-{}", std::process::id()));
