//! iuv-repl：CLI 调试前端（任务书 12-mod-iuv-repl.md，Agent C 属主）。
...
use iuv_core::{apply_keymap, Config, Effect, Engine, Key, Session, SessionEnd};
...
fn print_usage() {
    eprintln!("用法：iuv-repl <dict.imedic>");
    eprintln!("       iuv-repl <dict.imedic> --batch <拼音串>");
}
