...
mod dbg_investigate {
    use super::*;
    use crate::api::ImeEngine as _IE;

    fn dump(raw: &str) {
        println!("======== raw={}", raw);
        if !std::path::Path::new("../../data/iuv.imedic").exists() {
            println!("(skip: no local dict)");
            return;
        }
        let d = iuv_data::load(std::path::Path::new("../../data/iuv.imedic")).unwrap();
        let e = RimeEngine::new(std::sync::Arc::new(d), &crate::Config::default());
        // 内部复刻：seg/origins
        let seg = {
            let mut plans = e.schema.segment(raw);
            if plans.len() > 1 {
                let mut sc: Vec<(u32, usize)> = plans
                    .iter()
                    .enumerate()
                    .map(|(i, p)| {
                        let k = p.join("'");
                        (e.dict.exact(&k).first().map(|x| x.weight).unwrap_or(0), i)
                    })
                    .collect();
                sc.sort_by(|a, b| b.0.cmp(&a.0).then(a.1.cmp(&b.1)));
                plans = sc.into_iter().map(|(_, i)| plans[i].clone()).collect();
            }
            plans.into_iter().next().unwrap_or_default()
        };
        ...
        println!("seg={:?} origins={:?}", seg, origins);

        let lower = raw.to_lowercase();
        let g = syllabifier::build_graph(&lower, &e.syllables, MAX_SYLLABLE_LEN);
        let bks = translator::collect_buckets(&e.dict, &g, MAX_WORD_SYLLABLES, &origins, |_, _| false);
        println!("-- buckets (origin=0):");
        for ((s, ee), slot) in bks.iter() {
            if *s != 0 { continue; }
            for b in slot.iter().take(3) {
                println!("  (0,{}) {} w={} c{} x{} p{}", ee, b.entry.word, b.entry.weight, b.class, b.exact, b.parts);
            }
        }
        // 词流（复刻 translate 排序）
        let tr = e.translate(&EngineCtx { preceding_text: "" }, &PendingInput { raw });
        println!("-- stream top10:");
        for c in tr.candidates.iter().take(10) {
            println!("  {} kind{:?} seglen{}", c.text, c.kind, c.seg_len);
        }
        // 句通道内部
        let rest_all = {
            let ne: Vec<&String> = seg.iter().filter(|s| !s.is_empty()).collect();
            ne.len() >= 2 && ne[..ne.len() - 1].iter().all(|s| e.is_syllable(s))
        };
        println!("rest_all_syllables={}", rest_all);
        let filtered: translator::Buckets = bks
            .iter()
            .map(|(k, slot)| (*k, slot.iter().filter(|b| b.class != 1).cloned().collect()))
            .collect();
        if let Some(wg) =
            translator::build_poet_graph(&filtered, g.farthest, |w| e.lm.log_prob(None, "", w))
        {
            let edges: usize = wg.values().map(|m| m.len()).sum();
            let s = poet::make_sentence(&wg, g.farthest, "", poet::Strategy::DynamicProgramming);
            println!("wg edges={} sentence={:?}", edges, s.map(|x| x.words.join("")));
        } else {
            println!("wg=None (gate blocked)");
        }
    }

    #[test]
    fn investigate_deguo() {
        dump("deguolaoshixihuanchishuiguo");
    }

    #[test]
    fn investigate_after_teacher() {
        dump("laoshixihuanchishuiguo");
    }

    #[test]
    fn investigate_after_xi_drop() {
        dump("shixihuanchishuiguo");
    }
}
