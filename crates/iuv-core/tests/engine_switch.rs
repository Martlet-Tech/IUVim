//! rime 单引擎集成测试（39-rime-pipeline.md 收尾：classic 已删，rime 为唯一核心）。
//! `Engine::new` 内部装配 rime 核心，`start_session` 产出即 rime 会话。

use iuv_core::{Config, Engine, Key, SessionEnd};

fn rime_wired() -> std::sync::Arc<Engine> {
...
/// 默认装配即 rime 核心：候选带 score（rime 全量填充）、词优先无 Sentence
/// （可靠精确词闸门）。
#[test]
fn engine_uses_rime_core_by_default() {
